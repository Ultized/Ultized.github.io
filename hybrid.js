let cameras = [
  {
    ...
    width: 1959,
    height: 1090,
    position: [-0.033399276435375214, -1.4339958429336548, -2.5764939785003662],
    rotation: [
      [0.99891865253448486, 0.0062499353662133217, 0.04593866690993309],
      [-0.0095412731170654297, 0.99737495183944702, 0.071779415011405945],
      [-0.045369450002908707, -0.072140239179134369, 0.99635612964630127],
    ],
    fy: 3704.799560546875,
    fx: 3704.799560546875,
  },
  {
    id: 0,
    img_name: "00001",
    ...
    position: [-0.033399276435375214, -1.4339958429336548, -2.5764939785003662],
    rotation: [
      [0.99891865253448486, -0.0095412731170654297, -0.045369450002908707],
      [0.0062499353662133217, 0.99737495183944702, -0.072140239179134369],
      [0.04593866690993309, 0.071779415011405945, 0.99635612964630127],
    ],
    fy: 3704.799560546875,
    fx: 3704.799560546875,
  },
...
function createWorker(self) {
  ...
  function runSort(viewProj, cur_t) {
    ...
    let cur_pos = new Float32Array(3);


    for (let i = 0; i < vertexCount; i++) {
      let delta_t = cur_t - center_t[i];
      cur_pos[0] = positions[3 * i + 0] + motions[10 * i + 0] * delta_t + motions[10 * i + 3] * delta_t * delta_t + motions[10 * i + 6] * delta_t * delta_t * delta_t ;
      cur_pos[1] = positions[3 * i + 1] + motions[10 * i + 1] * delta_t + motions[10 * i + 4] * delta_t * delta_t + motions[10 * i + 7] * delta_t * delta_t * delta_t ;
  ...
  self.onmessage = (e) => {
    if (e.data.texture) {
      ...
      center_t = new Float32Array(vertexCount);
      for (let i = 0; i < vertexCount; i++) {
        ...
        center_t[i] = unpackHalf2x16(texture[16 * i + 8 + 7])[0];
      }
      throttledSort();
