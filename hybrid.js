...
function createWorker(self) {
  ...
  function runSort(viewProj) {
    ...
    let depthInv = (vertexCount) / (maxDepth - minDepth);
    let counts0 = new Uint32Array(vertexCount);
    for (let i = 0; i < vertexCount; i++) {
      sizeList[i] = ((sizeList[i] - minDepth) * depthInv) | 0;
      counts0[sizeList[i]]++;
    }
    let starts0 = new Uint32Array(vertexCount);
    for (let i = 1; i < vertexCount; i++) starts0[i] = starts0[i - 1] + counts0[i - 1];
